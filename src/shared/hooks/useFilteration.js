...
const useFiltration = () => {
  ...
  const initialDailyWear =
    searchParams.get("dailyWear")?.split(",").filter(Boolean) || [];
  const initialMetalColorSlug =
    searchParams.get("metalColorSlug")?.split(",").filter(Boolean) || [];
  const initialIsPaid = searchParams.get("isPaid") || "";

  const initialMetalType =
    searchParams.get("metalType")?.split(",").filter(Boolean) || [];
  const initialPurity =
    searchParams.get("purity")?.split(",").filter(Boolean) || [];
  const initialGender =
    searchParams.get("gender")?.split(",").filter(Boolean) || [];
  const initialPriceRange = searchParams.get("priceRange") || "";
  const initialWeightRange = searchParams.get("weightRange") || "";
  const initialGiftRange = searchParams.get("giftRange") || "";
  const initialIsEnamel = searchParams.get("isEnamel") || "";
  ...
  const [filters, setFilters] = useState({
    ...
    productStyleSlug: Array.isArray(initialProductStyleSlug)
      ...
      : [],
    caratWeight: Array.isArray(initialCaratWeight) ? initialCaratWeight : [],
    ...
    dailyWear: Array.isArray(initialDailyWear) ? initialDailyWear : [],
    metalColorSlug: Array.isArray(initialMetalColorSlug)
      ? initialMetalColorSlug
      : [],
    isPaid: initialIsPaid,
    metalType: Array.isArray(initialMetalType) ? initialMetalType : [],
    purity: Array.isArray(initialPurity) ? initialPurity : [],
    gender: Array.isArray(initialGender) ? initialGender : [],
    priceRange: initialPriceRange,
    weightRange: initialWeightRange,
    giftRange: initialGiftRange,
    isEnamel: initialIsEnamel,
  });
  ...
  useEffect(() => {
    ...
    if (searchParams.get("debouncedSearch")) {
      newFilters.debouncedSearch =
        searchParams.get("debouncedSearch")?.filter(Boolean) || [];
    }
  ...
  useEffect(() => {
    ...
    if (filters.weightRange) params.set("weightRange", filters.weightRange);
    if (filters.giftRange) params.set("giftRange", filters.giftRange);
    if (filters.isEnamel) params.set("isEnamel", filters.isEnamel);
  ...
  const handleFilterMultipleChangeHook = (event) => {
    ...
    if (type === "checkbox") {
      setFilters((prevFilters) => {
        ...
        // Convert value to array if it's a string of comma-separated values
        ...
        if (checked) {
          ...
          // Remove all the values that were in this option
          return {
            ...prevFilters,
            [name]: currentValues.filter(
              (item) => !valuesToHandle.includes(item)
            ),
          };
        }
      });
