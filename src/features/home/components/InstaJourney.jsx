...
export default function InstaJourney({ cmsData }) {
  const sectionTitle = cmsData?.sectionTitle ?? "Join the Journey on Instagram";
  const cards = cmsData?.posts ?? DEFAULT_POSTS;
  return (
    <section className="bg-white py-8 md:py-12 px-4 sm:px-8 md:px-12 lg:px-16">
      {/* Heading */}
      <div className="flex items-center justify-center gap-3 mb-8 md:mb-12">
        <div className="flex items-center">
          <img src="/home/why-diamantra/Line-left.svg" className="w-10 sm:w-16 md:w-24 h-[2px]" alt="" loading="lazy" />
          <svg width="12" height="12" viewBox="0 0 10 10" fill="none">
            <polygon points="0,5 5,0 10,5 5,10" fill="#000000" />
          </svg>
        </div>
        <h2 className="font-heading italic text-xl sm:text-2xl md:text-[30px] text-charcoal whitespace-nowrap">
          {sectionTitle}
        </h2>
        <div className="flex items-center">
          <svg width="12" height="12" viewBox="0 0 10 10" fill="none">
          ...
          <img src="/home/why-diamantra/Line-right.svg" className="w-10 sm:w-16 md:w-24 h-[2px]" alt="" loading="lazy" />
        </div>
      </div>

      {/* Desktop grid — 5 columns */}
      <div className="hidden md:grid grid-cols-5 gap-5 max-w-8xl mx-auto">
        {cards.map((card, i) => (
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="flex flex-col min-w-0">
                ...
                <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                  {card.originalPrice && (
                    <span className="text-[11px] text-[#c57f82] line-through">{card.originalPrice}</span>
                  )}
                  <span className="text-sm font-semibold text-maroon">{card.price}</span>
                </div>
              </div>
            </div>
          </div>
