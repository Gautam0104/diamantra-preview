...
export default function WhyDiamantraMobile({ cmsData }) {
  ...
  return (
    <section className="bg-[#faf5f0] h-auto md:h-[481px] flex flex-col md:overflow-hidden px-3 xs:px-6 sm:px-10 md:px-16 lg:px-24 mt-12">
      {/* Heading */}
      <div className="flex items-center justify-center gap-4 py-5 shrink-0">
        ...
        <h2 className="font-heading text-[20px] text-charcoal whitespace-nowrap -tracking-[2%] leading-[50.09px]">
          {sectionTitle}
        </h2>
        <div className="flex items-center">
          <svg width="12" height="12" viewBox="0 0 10 10" fill="none">
      ...
      <div className="grid grid-cols-2 md:hidden gap-x-4 gap-y-4 pb-6 px-2 shrink-0">
        {[...leftFeatures, ...rightFeatures].map(({ img, title }) => (
          <div key={title} className="flex flex-col md:flex-row items-center gap-3">
            <img src={img} alt={title} className="w-26 h-26 object-contain shrink-0" loading="lazy" />
            <span className="font-semibold uppercase text-[12px] text-charcoal leading-[17px] tracking-[10%] text-center max-w-[120px]">
              {title}
            </span>
          </div>
        ))}
