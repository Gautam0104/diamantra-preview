...
export default function ShopByCategory({ categories = [] }) {
  ...
  return (
    <section className="flex flex-col py-8 md:py-12 ">
      {/* Header row */}
      <div className="flex items-center justify-between py-5 max-w-8xl mx-auto px-4  md:px-6 w-full">
        <div className="flex items-center gap-3">
          <h2 className="font-heading text-xl sm:text-2xl md:text-[30px] text-charcoal whitespace-nowrap">
            Shop by category
          </h2>
        ...
        <div className="flex items-center gap-2">
          <button className="cat-prev hidden md:flex w-9 h-9 rounded-full border border-gray-300 items-center justify-center hover:border-charcoal transition-colors cursor-pointer">
            <ArrowLeft size={18} />
          </button>
          <button className="cat-next hidden md:flex w-9 h-9 rounded-full border border-gray-300 items-center justify-center hover:border-charcoal transition-colors cursor-pointer">
            <ArrowRight size={18} />
          </button>
          <Button text="More Categories" onClick={() => navigate("/shop-all")} />
        </div>
      </div>

      {/* Slider */}
      <div className="max-w-8xl mx-auto px-4  md:px-6 w-full">
        <Swiper
          modules={[Navigation]}
